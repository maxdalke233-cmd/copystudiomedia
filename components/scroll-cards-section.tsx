  ...
  return (
    <div className="w-full rounded-2xl overflow-hidden select-none" style={{ background: "radial-gradient(ellipse at 60% 50%, rgba(160,40,10,0.28) 0%, #0a0604 70%)", border: "1px solid rgba(255,255,255,0.07)" }}>
      ...
      <div className="flex flex-col gap-3 px-4 pt-3 pb-4">
        {/* Header */}
        <div className="flex items-center gap-1.5">
          <span className="text-[9px] font-mono text-white/30 tracking-widest uppercase">Performance vs Branche</span>
        ...
        {/* Bar chart */}
        <div className="flex items-end gap-5" style={{ height: 90 }}>
          {/* Branche */}
        ...
        <div className="flex gap-5">
          <div className="flex-1 flex flex-col items-center gap-0.5">
            <span className="font-heading text-[22px] font-black text-white/30 leading-none">2.1×</span>
...
function ClockAnimation() {
  ...
  return (
    <div className="w-full rounded-2xl overflow-hidden select-none" style={{
      ...
      {/* Header */}
      <div className="flex items-center justify-between px-4 pt-3 pb-1">
        <span className="text-[9px] font-mono text-white/30 tracking-widest uppercase">Anfragen-Uhr</span>
        <div className="flex items-center gap-1.5">
          <motion.div
      ...
      <div className="flex flex-col gap-1.5 px-4 pt-3 pb-4">
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:"3px" }}>
          {["Mo","Di","Mi","Do","Fr","Sa","So"].map(d => (
            <span key={d} className="text-center text-[7px] font-mono text-white/20">{d}</span>
          ))}
        </div>
        {/* 3×7 Day grid */}
        <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:"3px" }}>
  ...
  return (
    <div className="w-full rounded-2xl overflow-hidden select-none" style={{ background: "radial-gradient(ellipse at 60% 50%, rgba(160,40,10,0.28) 0%, #0a0604 70%)", border: "1px solid rgba(255,255,255,0.07)" }}>
      ...
      <div className="flex flex-col gap-2 px-4 pt-3 pb-4">
        {/* Header */}
        <span className="text-[9px] font-mono text-white/30 tracking-widest uppercase">Organische Anfragen / Monat</span>
        {/* 7×2 grid */}
  ...
  return (
    <div className="w-full flex flex-col gap-1.5 select-none">
      ...
      <div className="flex items-center gap-2 rounded-full bg-white/[0.06] border border-white/[0.08] px-3 py-1.5 mb-0.5">
        ...
        <span className="text-[10px] text-white/45 truncate">handwerker münster</span>
      </div>
      {/* Position label */}
...
const PEEK        = 60;
const CONTAINER_H = CARD_H + PEEK * 2;

/* ─── Desktop carousel card (unchanged) ──────────────────────── */
function CarouselCard({
  ...
  const virtualActive = useTransform(scrollProgress, [0, 1], [0, TOTAL - 1]);
  const dist          = useTransform(virtualActive, (v) => Math.abs(index - v));
  const opacity       = useTransform(dist, [0, 1, 2], [1, 0.6, 0.38]);
  const borderOpacity = useTransform(dist, [0, 0.4, 1], [1, 0, 0]);

  return (
    <div className="relative shrink-0 w-full" style={{ height: CARD_H }}>
      <div className="absolute inset-0 rounded-3xl pointer-events-none" style={{ background: "rgba(255,255,255,0.07)" }} />
      <motion.div className="absolute inset-0 rounded-3xl overflow-hidden pointer-events-none" style={{ opacity: borderOpacity }}>
        <motion.div
          className="absolute"
...
/* ─── Mobile static card ──────────────────────────────────────── */
function MobileCard({ index, children }: { index: number; children: React.ReactNode }) {
  const ref = useRef<HTMLDivElement>(null);
  const inView = useInView(ref, { margin: "-30px 0px" });
...
export default function ScrollCardsSection() {
  ...
  const columnY = useTransform(scrollYProgress, [0, 1], [`${PEEK}px`, `${PEEK - (TOTAL - 1) * STEP}px`]);
    ...
    <>
      ...
      <section ref={ref} className="hidden md:block relative bg-black" style={{ minHeight: "620vh" }}>
        ...
        <div className="sticky top-0 h-screen overflow-hidden flex flex-col items-center px-6">
          ...
          >
            <motion.div className="flex flex-col" style={{ y: columnY, gap: CARD_GAP }}>

              <CarouselCard {...p} index={0}>
                <div className="flex-1 min-w-0">
